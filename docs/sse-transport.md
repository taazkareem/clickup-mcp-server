...
| `HOST` | Binding address (use 0.0.0.0 for Docker) | `127.0.0.1` |
...
| -------- | ------ | ----------- |
| `/mcp` | POST/GET | Main Streamable HTTP endpoint for MCP communication |
| `/sse` | GET | Legacy SSE connection endpoint |
| `/health` | GET | Health check and server status endpoint |
| `/.well-known/mcp/server-card.json` | GET | Auto-discovery endpoint for MCP clients |

## Authentication Headers

When using the remote transport, you must provide the following headers in your client's configuration:

| Header | Required | Description |
| ------ | -------- | ----------- |
| `X-ClickUp-Key` | Yes | Your ClickUp Personal API Token |
| `X-ClickUp-Team-Id` | Yes | Your ClickUp Workspace (Team) ID |
| `X-License-Key` | Yes | Your Premium License Key |
| `X-Enabled-Tools` | No | Comma-separated list of tools to enable |
| `X-Disabled-Tools` | No | Comma-separated list of tools to disable |
